...
{
	uint WorldVersion;
	asIScriptObject* DescriptorReference;
};
...
struct Critter
{
    ...
    const uint16 ProtoId;
    const uint16 LastWorldX;
    const uint16 LastWorldY;
    const uint16   KindIndex;
    const uint16   KindQIndex;
    const uint   KindHierarchy;
	...
	ScriptDescriptor GroupDescriptor;
    const uint   Reserved20[ 3 ];
...
struct MapObject
{
	...
	int SceneryGetParam( uint param )
	{
		if( 2 == MapObjType && param < MScenery.ParamsCount )
			return MScenery.Param[ param ];
...
struct Map
{
    ...
    struct
    {
		...
		ScriptDescriptor ScriptClass;
		uint         Index;
        const uint   Reserved[ 17 ];
	...
	ScriptString* script_GetName() const { return &ScriptString::Create( Proto->Name.c_str() ); }
	uint GetEventFuncId( uint index ) const 
	{
		if( index >= MAP_EVENT_MAX )
			return 0;
		return FuncId[ index ]; 
	}
	
    #ifdef __SERVER
	void script_SetIndex( uint value ){ Data.Index = value; }
	uint script_GetIndex( ) { return Data.Index; }
...
struct Location
{
    ...
    struct
    {
        ...
        ScriptDescriptor ScriptClass;
        uint   HashStaticName;
        const uint   Reserved3[ 56 ];
    } Data;

	uint script_GetHashStaticName( ) const { return Data.HashStaticName; }
	void script_SetHashStaticName( uint value ) { Data.HashStaticName = value; }
